...
def consilium_stats():
    ...
    cons_by_name =  db.session.query(Consnames.Consnames)
    consiliums = list()
    for item in cons_by_name:
        cons = consStats()
        cons.cons = item
        cons.manager = db.session.query(User.User).filter(User.User.id == Usercons.Usercons.user_id, Usercons.Usercons.manager==True, Usercons.Usercons.cons_num==item.id).first()
        cons.users = db.session.query(User.User).filter(User.User.id == Usercons.Usercons.user_id, Usercons.Usercons.manager==False, Usercons.Usercons.cons_num==item.id).all()
        pics = db.session.query(Consdata.Consdata.pic_id).filter(Consdata.Consdata.cons_num==item.id)
        cons.pics_by_symp = db.session.query(Symptom.Symptom.symptom_name, db.func.count(Symptom.Symptom.symptom_name).label('total')) \
            .join(Consilium.Consilium)\
            .filter(Symptom.Symptom.diagnos==True, Consilium.Consilium.pic_id.in_(pics)) \
            .group_by(Symptom.Symptom.symptom_name).order_by(db.desc('total'))
        consiliums.append(cons)
    ...
    pics_by_symp = db.session.query(Symptom.Symptom.symptom_name, db.func.count(Symptom.Symptom.symptom_name).label('total')) \
        ...
        .group_by(Symptom.Symptom.symptom_name).order_by(db.desc('total'))
    return render_template('/consilium/stats.pug', cons_pics=cons_pics_count, pics_by_symp=pics_by_symp, admin=current_user.admin, consiliums=consiliums)

@userApp.route('/consilium_view/', methods=['POST'], defaults={'page': 0})
@userApp.route('/consilium_view/<int:cons_num>/<int:page>', methods=['GET'])
@login_required
def consilium_view_results(cons_num, page):
    ...
    showConf = db.session.query(Consilium.Consilium.pic_id).group_by(Consilium.Consilium.pic_id)
    pics = pics.filter(Picture.Picture.id.in_(showConf))
    ...
    for pic in pics:
        ...
        for symptom in symptoms:
            symp = picSymps()
            users = db.session.query(User.User).filter(User.User.id.in_(users_cons),
                                                       User.User.id == Recognized.Recognized.user_id,
                                                       Recognized.Recognized.symp_id == symptom.id,
                                                       Recognized.Recognized.pic_id == pic.id)
            symp.users = list()
            for usr in users:
                symp.users.append(usr)
        ...
        if (len(list(cons)) > 0):
            for cns in cons:
                form.consilium.append(cns.symp_id)
        pics_list.append(form)
    manager = db.session.query(User.User).filter(User.User.id == Usercons.Usercons.user_id,
                                                      Usercons.Usercons.manager == True,
                                                      Usercons.Usercons.cons_num == cons_num).first()
    cons_users = db.session.query(User.User).filter(User.User.id == Usercons.Usercons.user_id,
                                                    Usercons.Usercons.manager == False,
                                                    Usercons.Usercons.cons_num == cons_num).all()

    consname = db.session.query(Consnames.Consnames).get(cons_num)
    return render_template('/consilium/view.pug', admin=current_user.admin, pagination=pagination, pictures=pics_list,
                           cons_number=cons_num, consname=consname, cons_users=cons_users, manager=manager)
